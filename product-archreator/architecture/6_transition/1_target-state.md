...
| `GAP4` | **A pending relationship reads as a live one** | The notation says a dashed edge means Pending. `DOBJ4` carries a `pending` field and `ACMP7` reads a `Pending` marker from a relationship's notes column — and **no relationship in any tree writes one**, so all 635 edges are projected as live while 35 are still drawn dashed: 24 in the layer documents and 11 in this folder. Anything reading the projection as current state is wrong about them. The mechanism arrived with [initiative 8](../scope/8_declare-the-relationships-and-let-the-graph-be-walked.md); the data never went through it | `PLAT1` |
