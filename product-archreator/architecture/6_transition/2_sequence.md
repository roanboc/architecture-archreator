...
```mermaid
flowchart LR
  i7[["≡ 7 — Declare the relationships [PLAT1]"]]:::plateau
  i8[["≡ 8 — Walk the model [PLAT2]"]]:::plateau
  i9[["≡ 9 — Federate the graph [PLAT3]"]]:::plateau
  i10[["≡ 10 — Cross the boundary [PLAT4]"]]:::plateau
  i11[["≡ 11 — Make it readable [PLAT5]"]]:::plateau
  i12[["≡ 12 — Answer one question [PLAT6]"]]:::plateau

  i7 -.->|a graph worth drawing| i8
  i8 -.->|a reader worth pointing at more than one| i9
  i9 -.->|somewhere to resolve a foreign identifier| i10
  i8 -.->|a reader worth making readable| i11
  i11 -.->|its prose and its vocabulary, kept| i12
...
| - | ---------- | ------ | ------- | ---- | ----- |
| 7 | [Declare the relationships, and let the graph be walked](../scope/7_declare-the-relationships-and-let-the-graph-be-walked.md) | `GAP1`, `GAP2`, `GAP3`, `GAP4` | `PLAT1` | 2 | In flight |
| 8 | [Walk the model](../scope/8_walk-the-model.md) | `GAP5`, `GAP6` | `PLAT2` | 2 and 3, both delegated ([decision 2](../decisions/2_the-requester-delegates-the-remaining-gates.md)) | **Written off** — see [decision 4](../decisions/4_the-graph-portal-is-retired.md); `GAP6`'s answer, the shared traversal, is kept |
| 9 | [Federate the graph](../scope/9_federate-the-graph.md) | `GAP7`, `GAP8` | `PLAT3` | 2 and 3, both delegated ([decision 2](../decisions/2_the-requester-delegates-the-remaining-gates.md)) | In flight |
| 10 | [Cross the boundary](../scope/10_cross-the-boundary.md) | `GAP9` | `PLAT4` | 2 and 3, both delegated ([decision 2](../decisions/2_the-requester-delegates-the-remaining-gates.md)) | In flight |
| 11 | [Make it readable](../scope/11_make-it-readable.md) | `GAP10`–`GAP13` | `PLAT5` | 1 by the Requester's own direction; 2 and 3 delegated ([decision 3](../decisions/3_the-navigator-earns-its-own-initiative.md)) | **Written off** — its plateau is abandoned, and what it built is kept |
| 12 | [Answer one question](../scope/12_answer-one-question.md) | `GAP14`–`GAP16` | `PLAT6` | 1 by the Requester's own direction; 2 and 3 delegated ([decision 4](../decisions/4_the-graph-portal-is-retired.md)) | In flight |
...
**Initiative 10 is last, and that is a change from the first instinct.** The
...
initiative 9. Attempting it earlier means either a validator that clones every
...
Initiatives 8 and 11 built a graph navigator and then made it readable. Both
...
projection, the shared traversal, and the prose excerpts initiative 11 added,
which are the body of every brief. A brief is assembled from the work those two
initiatives did; only the reader changed.

**The lesson is worth more than the code.** Initiative 8 recorded, in its own
...
- **The relationship tables turn out to be unwritable by hand.** Initiative 7
  ...
  projection and belongs in initiative 8.
- **A second organization appears before initiative 9.** The index in `PLAT3`
