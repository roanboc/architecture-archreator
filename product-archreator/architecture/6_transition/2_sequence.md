...
flowchart LR
  ...
  i2 -->|the export and the brief are proven| i7
...
| 7 | **Serve one question** — the export's fields stated as a contract a host can read, and the brief reachable by a reader who is not in a terminal, on whatever platform hosts it | `GAP11`, `GAP12` | `PLAT4` | 2 — the export and the brief are proven; the same authorization, because a host holds a copy | **Planned** |
...
site can be built locally and never published; a host holds a copy by
definition. Both wait on the same authorization, and the host waits on the
site only where the Requester wants the site first.
...
- **Whether 7 precedes 6.** Nothing in serving one question needs the site.
  If the hosted platform is the priority, initiative 7 moves ahead and the
  site follows, or never comes.
...
- **The host turns out to want the site rather than the brief.** Then
  initiative 6 moves ahead of 7 and 7 consumes what 6 publishes, which is a
  reordering rather than a new plateau.
