...
| 4_application | **`ACMP2` (Bootstrap) gains a realizing artifact** — `templates/`, the scaffold it emits, now including `scripts/`. The scaffold was never modeled as a component of its own and does not become one: it is what `ACMP2` produces, and `P4`/`P5` argue against inventing an element for it. **`ACMP13` and `ACMP15` changed and relocated** — both check scripts key on the model directory name, learn to skip the narrative folders inside it, find their own repository root, and move into the scaffold so every project inherits them |
...
| **Target** (delivered) | `org-archreator/` and `product-archreator/` (with `site/` nested inside it), each declaring its depth, each keeping its whole architecture — layers, scope, decisions, history — under one `architecture/`. The scaffold ships inside the skill that emits it, validators included, so a generated project can check its own model from the first commit. The portability boundary is written down and testable. The root holds archreator's own `CLAUDE.md` and `README.md`, not the template's |
...
- **Deliverables:** `architecture/` → `architecture/` in all three project trees,
  with `architecture/scope/`, `architecture/decisions/` and the open-questions log rising to sit beside
...
- **Deliverables:** the contents of `docs/` relocated to
  ...
  The scaffold also carries `scripts/` — both validators — so a generated
  project can check its own model from its first commit. They live there
  **only**: archreator runs the same copy it ships, which is why the
  answer to open question 1 stays true for the tooling as well as the
  documents.
...
- **Deliverables:** [`scripts/check_model.py`](../../../.claude/skills/project-bootstrap/templates/scripts/check_model.py)
  ...
  an `ea/` folder; it learns the new name. [`scripts/check_links.py`](../../../.claude/skills/project-bootstrap/templates/scripts/check_links.py)
...
- **The scaffold ships no `CONTRIBUTING.md`.** Its one reference to the
  process document points at archreator's published copy rather than a file
  the project owns. Defensible — the process belongs to the method — but a
  project that wants to amend how *it* works has nowhere to write that down.
- **The validators assume Python.** They now ship with the scaffold, so
  every generated project can check its own model from the first commit —
  but a project with no Python available inherits two files it cannot run.
  Nothing detects that; the bootstrap checklist simply asks for them to
  pass.
