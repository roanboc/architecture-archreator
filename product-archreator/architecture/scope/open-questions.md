...
| 12 | Does a level of a process or capability catalogue always get its own document, or only once it passes the fifteen-element threshold? | **The threshold**, reusing the number `architecture-doc-style` already applies to diagrams: below it a level is a column, above it a document. The request that prompted the change asked for a document per level outright, so this is a deviation and not an interpretation of silence | [14_a-model-a-human-can-read.md](./14_a-model-a-human-can-read.md) |
| 11 | How does a document in one project reference an element owned by another? | **Still open.** The workaround in use is to name the owning skill and section in prose rather than the identifier. A qualified form was proposed and declined at Gate 2 — see [scope document 11](./11_referencing-across-models.md) for the full analysis, including why allocated prefixes and a central catalogue were refused. Revisit when a second repository actually exists | [10_what-belongs-at-which-tier.md](./10_what-belongs-at-which-tier.md) |
