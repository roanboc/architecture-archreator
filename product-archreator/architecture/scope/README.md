...
| 11 | [11_referencing-across-models.md](./11_referencing-across-models.md) | `claude/product-1-roadmap-74giay` | A qualified form for citing an element another model owns — `product-archreator:RULE11` — extending the domain rule one level out, with the validator degrading rather than failing when the parent model is absent |
