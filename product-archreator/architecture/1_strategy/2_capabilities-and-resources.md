...
| -- | ---------- | ---------------- | ----------- | -------- |
| `CAP1` | **Discover a subject from nothing** | Turning a company or an application nobody has modeled into canvases, a strategy layer and a domain split, each stopped at its gate — and, where the subject was already running, sweeping its estate into the four layers below the strategy, with the documents it was built from filed and indexed beside the model | `establish-project`, `discover-business-model`, `discover-strategy`, `model-domains`, `discover-current-landscape` | Established |
