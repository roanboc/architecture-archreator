...
| -- | ------- | ---- | ----------- |
| `ASVC1` | **Method execution** | Walks a requirement through the layers, runs the discovery conversations, and stops at Direction and at Understanding where each applies: the skills, doing what skills do | [`ACMP1`] The skill corpus |
...
| `ASVC7` | **Model interrogation** | Reads a project fresh, caching nothing, and answers what a change would touch, what names no realizing artifact, whether a granted gate ever moved a status line, which elements name a code path, and one focused question as a disposable brief | [`ACMP5`] The model reader, [`ACMP6`] The brief generator |
