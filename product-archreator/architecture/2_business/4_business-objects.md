...
flowchart TB
  ...
  bobj10["▧ The generated brief [BOBJ10]"]:::object
  ...
  bobj10 -->|reads, and never changes| bobj1
  bobj3 -->|records| bobj5
  bobj3 -->|changes| bobj1
...
an element, a relationship, a decision, an approval. A brief asserts nothing —
it decides what a reader is looking at, and every sentence in it was written
somewhere else. It is modeled anyway because it is a thing the method now
hands people, and because writing down what it *is not* is the only way the
boundary survives the next feature that makes writing tempting: the generator
reads the model and has no way to change it.

**It is also the one object here that is never committed.** The rest are files
in git, which is what makes them reviewable and versioned. A brief is a
snapshot of a revision, stamped with the one it came from and saying on its
first line that it is disposable — because a generated document that does not
announce itself is quoted eight months later as though somebody had approved
it. `RULE7` is the same rule for a portal; this is it for a document.
