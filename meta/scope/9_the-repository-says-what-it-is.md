...
and its directory names identify none of them. `docs/` is a template, `meta/`
is the method, `organization/` is the business, and `site/` is a channel. A
...
| --- | -------- | ------------------------------- |
| [`meta/open-questions.md`](../open-questions.md) #1 | Should the template repository be **generated** from the plugin, or hand-maintained alongside it? | **Answers it — generated.** The scaffold moves inside the skills that emit it, so the two cannot drift |
| [`meta/open-questions.md`](../open-questions.md) #5 (Resolved) | Where should archreator record its own architecture, given that `docs/` must stay blank for cloners? | **Supersedes it.** The premise dissolves — once the scaffold ships inside the skills, `docs/` no longer has to stay blank, and the reason `meta/` exists under that name goes with it |
| [`2_business-model-canvas.md`](../../organization/docs/ea/0_business-design/2_business-model-canvas.md#open-questions) #2 | How far should provider neutrality go in practice? | **Answers it.** The canvas says a decision record is needed _"before anything provider-specific enters a skill body"_ and _"before the question is settled by accident"_ — and the scaffold move would settle it by accident if taken first |
...
The subject is **the method**, so this document lives in `meta/scope/` — the
...
| 4_application | **`ACMP4` redefined** — the scaffold stops being "the empty tree at `docs/` that a cloner inherits" and becomes "the templates the skills emit", relocating into the skill directories. **`ACMP13` and `ACMP15` changed** — both check scripts key on the model directory name and must learn the new one |
...
| Document | Change |
| -------- | ------ |
| [`2_business-services.md`](../../organization/docs/ea/2_business/2_business-services.md) | `BSVC2`'s realizing artifacts are recorded as "`site/`, `meta/`, and this tree". Those are current-state facts stated as paths, so they change with the paths |
| [`2_business-model-canvas.md`](../../organization/docs/ea/0_business-design/2_business-model-canvas.md) | Open question 2 moves from Pending to answered, pointing at the decision record from WP1. `CH3` gains its tier: the marketplace is packaging, and a second one is additive |
...
| Gate 2 — Business | _awaiting_ | — | This document, and the two decision records drafted in WP1 |
| Gate 3 — Solution design | _to be asked at Gate 2_ | — | The check-script changes and the template placement, if the Requester opts in |
...
- **Deliverables:** `meta/decisions/6_the-portability-boundary.md` — the
  ...
  `meta/decisions/7_one-tree-per-federated-project.md` — the top-level layout
...
- **Deliverables:** `organization/` → `org-archreator/`; `meta/` →
...
- **Deliverables:** `docs/ea/` → `architecture/` in all three project trees,
...
- **Deliverables:** the contents of `docs/` relocated to
  `templates/` inside each emitting skill's directory —
  `project-bootstrap` at minimum; `scope-doc` and `decision-record` if they
  carry emitted shapes of their own, which WP4 confirms first. Root `docs/`
  retired. `RULE9` restated as the packaging constraint it is.
- **Outcome:** `meta/open-questions.md` #1 is answered — the scaffold is
...
- **Deliverables:** [`scripts/check_model.py`](../../scripts/check_model.py)
  ...
  — the unresolved-numbered-file rule is keyed on `docs/ea/` and follows.
...
- **Should the site keep its own model once nested, or fold into the
  ...
  [`meta/open-questions.md`](../open-questions.md).
