---
name: operating-model-discovery
description: Use when the initiative is modeling an organization rather than building a single application — a company, a department, or a service line whose operating model is itself the deliverable. Runs a question-driven discovery with the Requester over the Value Proposition Canvas and a Business Model Canvas per product, ending at an explicit business-model approval gate (Gate 0), then hands off to strategy-discovery to derive the enterprise architecture from it.
---
...
organization. Use `strategy-discovery` directly when it is a single
...
If none of that is true, stop and use `strategy-discovery`.

## How to run the conversation

The rules are the same as `strategy-discovery`'s, and they matter more here
...
- the strategy and key business layers derived from them via
  `strategy-discovery`;
- a scope document (`scope-doc` skill) whose EA-alignment table records the
