...
_Reached from `architecture-first-change` Step 2 when the project is Depth 2
or above, from `strategy-discovery` theme 4, and from
`operating-model-discovery` at the hand-off. `architecture-doc-style` governs
...
cut. This is `architecture-doc-style` § Consolidate before you enumerate
...
`strategy-discovery` theme 5 already works — produces nothing for the other
...
`realized by` is the grounding rule (`architecture-doc-style` § Grounding
...
fifteen-element threshold in `architecture-doc-style` § Diagrams come first
...
inventory table — per `architecture-doc-style`. The index README carries the
