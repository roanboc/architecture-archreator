...
flowchart TB
  ...
  subgraph BUS["Business layer"]
    actor(["⚇ The Requester (Human) [ACT1]"]):::business
    svc(["⬭ The method, published and installable [BSVC1]"]):::business
  end

  subgraph APP["Application layer"]
    app["⊞ The skill corpus and plugin manifest [ACMP1]"]:::application
  end

  subgraph TEC["Technology layer"]
    tech["⬒ The code host [NODE1]"]:::technology
  end

  goal -->|realized by| vs
  vs -->|realized by| svc
  actor -->|assigned to| svc
  svc -->|realized by| app
  app -->|runs on| tech
...
`0_business-design/1_value-proposition-canvas.md`
→ `0_business-design/2_business-model-canvas.md`,
and read the strategy layer as their consequence:
`1_strategy/1_motivation.md`
→ `1_strategy/3_value-stream.md`
→ `2_business/2_business-services.md`
→ `3_information/1_data-objects.md`
→ `4_application/2_application-components.md`
→ `5_technology/2_deployment.md`.
