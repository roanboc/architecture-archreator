...
What is read: every ``*.md`` under each ``architecture/`` folder of the repository
(one model, or one per tree), except the narrative
...
EXCLUDED_DIRS = {".git", ".archreator", ".model", "node_modules", ".venv"}
...
def models() -> list[Path]:
    """Every `architecture/` folder in the repository: one model, or one per tree."""
    return sorted(d for d in REPO_ROOT.rglob(MODEL_DIR) if d.is_dir() and not EXCLUDED_DIRS & set(d.relative_to(REPO_ROOT).parts))
...
def pages(not_started: re.Pattern[str]) -> list[Path]:
    ...
    for model in models():
        for path in sorted(model.rglob("*.md")):
            rel = path.relative_to(model.parent)
